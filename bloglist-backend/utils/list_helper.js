...
const totalLikes = (blogs) => {
  ...
  return blogs.length === 0 ? 0 : blogs.reduce(summingLikes, 0);
};
...
const mostBlogs = (blogs) => {
  ...
  const maxAuthor = Object.keys(authorsRanked).reduce((a, b) =>
    authorsRanked[a] > authorsRanked[b] ? a : b
  );
...
const mostLikes = (blogs) => {
  ...
  const authorsRanked = blogs.reduce(function (accLikesAuthor, currentBlog) {
    return (
      accLikesAuthor[currentBlog.author]
        ? (accLikesAuthor[currentBlog.author] =
            accLikesAuthor[currentBlog.author] + currentBlog.likes)
        : (accLikesAuthor[currentBlog.author] = currentBlog.likes),
      accLikesAuthor
    );
  }, {});
