...
const BlogView = () => {
  ...
  if (allBlogs) {
    ...
    const removeBlog = (event) => {
      event.preventDefault();
      if (
        window.confirm(`Do you really want to delete ${selectedBlog.title}?`)
      ) {
    ...
    return (
      <div className="px-4 mx-auto sm:w-1/2 w-full">
        <div className="bg-white rounded-md shadow-lg ps-8 pe-8">
          <div className="mb-3">
            <p class="pt-5 text-sm text-gray-600">{selectedBlog.author}</p>
            ...
            <ul className="mt-4">
              {selectedBlog.comments
                ? selectedBlog.comments.map((comment, id) => (
                    <li key={id} className="mt-1 ms-1 text-gray-600">
                      {comment}{" "}
                    </li>
                  ))
                : null}
