...
export const createBlog = (blogObject) => {
  return async (dispatch) => {
    try {
      ...
      dispatch(
        setNotification({
          message: `Error. Blog not created. Full error message: ${error}`,
          className: "error",
        })
      );
    }
...
export const addBlogComment = ({ comment, id }) => {
  ...
  return async (dispatch) => {
    try {
      ...
      dispatch(
        setNotification({
          message: "Succes! Blog comment added",
          className: "success",
        })
      );
    } catch {
      dispatch(
        setNotification({
          message: "Something went wrong adding comment. Full error:",
          className: "error",
        })
      );
    }
...
export const likeBlog = (likedBlog) => {
  return async (dispatch) => {
    try {
      ...
      dispatch(
        setNotification({
          message: "You can only like blogs when logged in",
          className: "error",
        })
      );
    }
